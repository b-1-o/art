...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUpRight, Check, ChevronDown, ExternalLink, Instagram, Menu, Minus, Plus, ShoppingBag, X } from 'lucide-react'
...
function Image({ artwork, className = '', priority = false }: { artwork: Artwork; className?: string; priority?: boolean }) {
  ...
  return <div className={'art-image ' + className} style={style}>{failed ? <div className="image-fallback"><span>{artwork.title.charAt(0)}</span><small>IMAGE PREVIEW</small></div> : <img src={artwork.image} alt={artwork.title} loading={priority ? 'eager' : 'lazy'} decoding="async" onError={() => setFailed(true)} />}<small className="image-credit"><a href={artwork.creditUrl} target="_blank" rel="noreferrer">{artwork.credit}</a></small></div>
}

function Card({ artwork, compact = false }: { artwork: Artwork; compact?: boolean }) {
  const price = artwork.status === 'Sold' ? 'SOLD' : artwork.status === 'Edition' ? 'FROM ' + '$' + artwork.price.toLocaleString() : '$' + artwork.price.toLocaleString()
...
function Works({ onNavigate }: { onNavigate: (p: string) => void }) {
  ...
  return <div className="page-wrap"><Intro eyebrow="01 / WORKS" title={<>A living catalogue<br /><em>of the studio.</em></>} body="Paintings, works on paper and objects. Each work is catalogued with medium, scale, year and availability." />
    <section className="section-tight catalogue"><div className="catalogue-toolbar"><div className="filter-row">{(['All', 'Painting', 'Works on Paper', 'Object'] as const).map((item) => <button className={category === item ? 'selected' : ''} key={item} onClick={() => setCategory(item)}>{item}</button>)}</div><label className="sort-select"><span>Sort</span><select value={sort} onChange={(e) => setSort(e.target.value as typeof sort)}><option>Featured</option><option>Newest</option><option>Price</option></select><ChevronDown size={14} /></label></div><div className="catalogue-grid">{items.map((a, i) => <Card key={a.id} artwork={a} compact={i % 3 === 1} />)}</div><div className="collection-list"><div className="eyebrow-row"><span>COLLECTIONS</span><span>03 / 03</span></div>{collections.map((c, i) => <div className="collection-row" key={c.title}><span>0{i + 1}</span><h3>{c.title}</h3><p>{c.description}</p><b>{c.count}</b><ArrowUpRight size={18} /></div>)}</div></section>
  </div>
}
...
function Contact({ onNavigate }: { onNavigate: (p: string) => void }) {
  ...
  return <div className="page-wrap"><Intro eyebrow="05 / CONTACT" title={<>Start a conversation<br /><em>with the studio.</em></>} body="For acquisitions, commissions, collaborations and learning sessions." /><section className="section-tight contact-grid"><div className="contact-copy"><p className="eyebrow">DIRECT</p><a href="mailto:studio@aer.art" className="contact-email">studio@aer.art <ArrowUpRight size={18} /></a><div className="contact-detail"><span>STUDIO HOURS</span><p>Tuesday — Friday<br />10:00 — 17:00 PT</p></div><div className="contact-detail"><span>SOCIAL</span><a href="https://instagram.com" target="_blank" rel="noreferrer">Instagram <Instagram size={14} /></a></div><button className="text-button" onClick={() => onNavigate('/shop')}>See available works <ArrowUpRight size={15} /></button></div><form className="contact-form" onSubmit={submit}><label><span>Your name</span><input required name="name" placeholder="Jane Doe" /></label><label><span>Email</span><input required type="email" name="email" placeholder="you@example.com" /></label><label><span>I’m writing about</span><select defaultValue="acquisition"><option value="acquisition">Acquiring a work</option><option value="commission">A commission</option><option value="learning">A learning session</option><option value="collaboration">A collaboration</option></select></label><label><span>Message</span><textarea required name="message" rows={7} placeholder="Tell the studio a little about what you’re looking for..." /></label><button className="dark-button" type="submit">{sent ? <>Message prepared <Check size={15} /></> : <>Prepare inquiry <Plus size={15} /></>}</button>{sent && <p className="form-success">Showcase form complete. Connect this handler to Formspree, Resend or your preferred backend for production.</p>}</form></section></div>
}
